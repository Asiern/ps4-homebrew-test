...
typedef struct
{
    ...
    unsigned char program_type; /* 0x0 PUP, 0x8 NPDRM Application, 0x9 PLUGIN, 0xC Kernel, 0xE Security Module, 0xF
                                   Secure Kernel */
    uint16_t padding;           /* Padding. */
    uint16_t header_size;       /* Header size. */
    uint16_t signature_size;    /* Metadata size? */
    uint64_t self_size;         /* Size of SELF. */
    uint16_t num_of_segments;   /* Number of Segments, 1 Kernel, 2 SL and Modules, 4 Kernel ELFs, 6 .selfs, 2 .sdll, 6
                                   .sprx, 6 ShellCore, 6 eboot.bin, 2 sexe. */
